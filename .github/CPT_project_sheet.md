...
This document is the living technical specification for the "Community Plus" plugin. It will be updated as development progresses.
...
1.  **Plugin Structure Setup:**
    * Create the plugin directory `community_plus/`.
...
3.  **Conditional Logic (within `commplus_setup_ucp_tabs`):**
...
6.  **Create the Album Manager UI Template (`template/ucp_album_manager.tpl`):**
    * This file now *only* contains the form elements for managing the albums (the loop, text inputs, textareas, and checkboxes). It does not contain any surrounding `<fieldset>` or submit button, as it will be injected into the main profile form.
...
1.  **Test Owner Albums Retrieval:** Verify that the function fetching albums for the UCP returns *only* the albums owned by the specified user.
