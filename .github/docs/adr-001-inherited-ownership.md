...
- A directly owned Community root can stay an ownership anchor without needing to remain a guest-visible album node; guest album totals now follow descendant visibility instead of leaking counts from mismatched child privacy.
...
- When an owner privatizes a directly owned Community root, treat that as a tree-level privacy intent for its descendant image-holder albums. CPT should repair already-broken private roots on later owner-facing requests instead of assuming the previous tree state was consistent.
