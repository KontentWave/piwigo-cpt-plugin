...
- `categories`: CPT updates privacy with per-row `UPDATE ... WHERE id = ... LIMIT 1` statements (parent + one per descendant when propagating) instead of core's recursive `set_cat_status()` / `get_subcat_ids()`; descendant discovery scans the whole `categories` table. Since 2026-07-19 the whole sequence (status + descendants + `user_access` sync) runs in a single verified transaction with rollback-on-failure.
...
- 2026-07-19 (fixes, round 2): audit R2 fixed — CPT privacy transitions are transactional (BEGIN/COMMIT/ROLLBACK via `pwg_query`) with per-step verification; on failure the transition rolls back entirely, no cache invalidation fires, and the caller shows a translatable error. Cross-plugin note: transactions are short-lived and only span CPT's own writes to `categories`/`user_access`.
