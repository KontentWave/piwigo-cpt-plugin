...
class core_privacy_toggle_maintain extends PluginMaintain
{
  ...
  function __construct($plugin_id)
  {
    ...
    $this->table = $prefixeTable . 'core_privacy_toggle';
    $this->dir = PHPWG_ROOT_PATH . PWG_LOCAL_DIR . 'core_privacy_toggle/';
  }
  ...
  function install($plugin_version, &$errors=array())
  {
    ...
    if (empty($conf['core_privacy_toggle']))
    {
      // conf_update_param well serialize and escape array before database insertion
      // the third parameter indicates to update $conf['core_privacy_toggle'] global variable as well
      conf_update_param('core_privacy_toggle', $this->default_conf, true);
    }
    else
    {
      $old_conf = safe_unserialize($conf['core_privacy_toggle']);
      ...
      conf_update_param('core_privacy_toggle', $old_conf, true);
    }
...
;');
    ...
    $result = pwg_query('SHOW COLUMNS FROM `'.IMAGES_TABLE.'` LIKE "core_privacy_toggle";');
    if (!pwg_db_num_rows($result))
    {
      pwg_query('ALTER TABLE `' . IMAGES_TABLE . '` ADD `core_privacy_toggle` TINYINT(1) NOT NULL DEFAULT 0;');
    }
  ...
  function uninstall()
  {
    ...
    conf_delete_param('core_privacy_toggle');

    // delete table
    pwg_query('DROP TABLE `'. $this->table .'`;');

    // delete field
    pwg_query('ALTER TABLE `'. IMAGES_TABLE .'` DROP `core_privacy_toggle`;');
